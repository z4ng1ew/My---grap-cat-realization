...
void grep(int counter_files, char *argv[], flags *options) {
    ...
    const char *pattern = options->fflag ? options->str_arguments_2 : options->str_arguments;
    regcomp(&reg, pattern, flags_regular);

    file = fopen(argv[optind], "r");
    if (file != NULL) {
        process_file(file, counter_files, argv, &reg, &first, &counter_str, options);
        fclose(file);
...
void process_file(FILE *file, int counter_files, char *argv[], regex_t *reg, regmatch_t *first, int *counter_str, flags *options) {
    while ((fgets(options->str_file, BUFFER, file)) != NULL) {
        ...
        if (!match && options->nflag) {
            ...
            if (counter_files > 1)
                printf("%s:%d:%s", argv[optind], options->nflag_count_str, options->str_file);
            if (counter_files > 0) sym_eof(options);
        }
        if (options->oflag == 1) oflag(match, *reg, *first, counter_files, argv, options);
        if (!match && !options->cflag && !options->lflag && !options->nflag && !options->oflag) {
            if (counter_files == 1 || options->hflag) printf("%s", options->str_file);
            if (counter_files > 1 && !options->hflag) printf("%s:%s", argv[optind], options->str_file);
...
void oflag(int match, regex_t reg, regmatch_t first, int counter_files, char *argv[], flags *options) {
    if (!match) {
        if (counter_files == 1)
            printf("%.*s\n", (int)(first.rm_eo - first.rm_so), options->str_file + first.rm_so);
        if (counter_files > 1)
            printf("%s:%.*s\n", argv[optind], (int)(first.rm_eo - first.rm_so), options->str_file + first.rm_so);
        char *remaining = options->str_file + first.rm_eo;
        while (!(regexec(&reg, remaining, 1, &first, 0))) {
            if (counter_files == 1)
                printf("%.*s\n", (int)(first.rm_eo - first.rm_so), remaining + first.rm_so);
            if (counter_files > 1)
                printf("%s:%.*s\n", argv[optind], (int)(first.rm_eo - first.rm_so), remaining + first.rm_so);
